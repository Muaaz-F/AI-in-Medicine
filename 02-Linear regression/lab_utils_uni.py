...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    # Initialize arrays for storing the values of p_hist and J_hist
    x = np.zeros(len(p_hist))
    y = np.zeros(len(p_hist))
    v = np.zeros(len(p_hist))
    
    # Fill in values from history
    ...
    # Set the bias b to a constant value
    fix_b = 100.0
    w_array = np.arange(-70000.0, 70000.0, 1000.0)  # Make sure it's float type
    cost = np.zeros_like(w_array, dtype=np.float64)  # Use float64 for cost array

    # Calculate cost for each weight value in w_array
    for i in range(len(w_array)):
        tmp_w = float(w_array[i])
        cost[i] = compute_cost(x_train.astype(np.float64), y_train.astype(np.float64), tmp_w, fix_b)
    ...
    tmp_b, tmp_w = np.meshgrid(np.arange(-35000.0, 35000.0, 500.0), np.arange(-70000.0, 70000.0, 500.0))
    z = np.zeros_like(tmp_b, dtype=np.float64)  # Initialize z array as float64

    # Calculate the cost for each combination of (w, b)
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train.astype(np.float64), y_train.astype(np.float64), tmp_w[i][j], tmp_b[i][j])

    # 3D plot for cost surface
